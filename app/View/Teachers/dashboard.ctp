...
<div class="teachers index">
	<div class="row" id="tutorial">
		<div class="col-md-12 switch_box">
        				<?php if($vPermit_data['vcite_permissions']['isEnabled'] == 1):?>
        					<input type="checkbox" name="my-checkbox" data-on-text="Enabled" data-off-text="Disabled" checked>
        					<p class="las_format">(NOTE: LAS' are disabled for students only)</p>
        				<?php else:?>
        					<?php if(!empty($vPermit_data['vcite_permissions']['list_of_sections_enabled']) || !empty($vPermit_data['vcite_permissions']['list_of_students_enabled'])):?>
        					<p class="las_format">VCITE is currently disable but some are permitted to access.</p>
        					<?php endif;?>

        					<input type="checkbox" name="my-checkbox" data-on-text="Enabled" data-off-text="Disabled">
        					<p class="las_format">(NOTE: LAS' are disabled for students only)</p>
        				<?php endif;?>
        			...
        			<script>
        				...
        				$("[name='my-checkbox']").on('switchChange.bootstrapSwitch', function (event, state) {
        				    ...
        				    //alert(this);
        				   	//alert(event);
        				    //alert(state);
        				    var url = 'http://192.168.12.5/vcite/teachers/vcite_permission/';
		...
		<div class="col-md-12">
			<ol class="breadcrumb">
				...
				<li class="active">
					...
					<a href="<?php echo $this->Html->url(array('controller'=>'teachers','action'=>'teacher_profile'));?>"><span class="glyphicon glyphicon-user"></span>&nbsp;&nbsp;Welcome <?php echo $this->Session->read('User.wholename');?>!</a>
				</li>
			</ol>
	...
	<div style="width:92%; margin:0 auto; text-align:right; padding:5px 5px 5px 5px;">
						<?php
							...
							echo '<input type="text" name="searcher" style="width:100%; font-style:italic; text-align:left; padding:5px;" placeholder="search a topic here"/>';
					...
					<div class="con_heading">Main Page</div>
						...
						<div class="con_body">
								...
								<div class="row" >
									...
									<div class="col-md-6">
										<h3>Subjects handled - <font style="font-size:12px;color:#fff; "><?php echo '<span style="background-color:#77CBDE; padding:4px 8px 4px 8px;
											border-radius: 10px 10px 10px 10px;
											-moz-border-radius: 10px 10px 10px 10px;
											-webkit-border-radius: 10px 10px 10px 10px;
										">'.$countSubjects2.'</span>';?> <a style="font-size:12px; font-style:italic;" href="<?php echo $this->Html->url(array('controller'=>'Subjects','action'=>'listsub'));?>">see all</a></font></h3>
											<ul class="los">
											<?php

												foreach($subjects2 as $subject){

													echo '<a href="'.$this->Html->url(array('controller'=> 'Subjects','action'=>'set_subject',$subject['Subject']['id'])).'">';
													echo '<li style="list-style:none;"><span class="glyphicon glyphicon-list-alt"></span> ';
													echo $subject['Subject']['title'];
													echo '</li></a>';

												}
												if(empty($subjects2) && !$isMobile)
													echo '<p style="font-style:italic;" ><a href="'.$this->Html->url(array('controller' => 'subjects','action' => 'add')).'" >create a new subject here.</a></p>';

											?>
											</ul>
									</div>
									<div class="col-md-6">
										<h3>Latest Assesstment of students - <font style="font-size:12px; color:#fff; "><?php echo '<span style="background-color:#77CBDE; padding:4px 8px 4px 8px;
											border-radius: 10px 10px 10px 10px;
											-moz-border-radius: 10px 10px 10px 10px;
											-webkit-border-radius: 10px 10px 10px 10px;
										">'.$countListOfAssTaken.'</span>';?> <a style="font-size:12px;font-style:italic;" href="<?php echo $this->Html->url(array('controller'=>'Students','action'=>'listloas'));?>">see all</a></font></h3>
											<ul >
											<?php echo $latestAssestmentOfStudents; ?>
											</ul>
									</div>
								...
								<div class="row">
									<div class="col-md-12">
										...
										<h3>E-books <label id="chevronUpDown"><span class="glyphicon glyphicon-chevron-down" style="cursor:pointer; border:solid 1px #cecece; padding:5px; margin-left:10px; font-size:14px;" ></span> <font style="cursor:pointer; border:solid 1px #cecece; padding:3px; font-size:12px;">show</font> </label></h3>
											<div class="col-md-12" id="eBookguide" style="font-style:italic; font-size:12px;">click the <strong>arrow</strong> or <strong>show</strong> button to display the electronic books</div>
								...
								<div class="row">
									<div class="col-md-12">
										...
										<h3>Created Pages <label id="chevronUpDown"><span class="glyphicon glyphicon-chevron-down" style="cursor:pointer; border:solid 1px #cecece; padding:5px; margin-left:10px; font-size:14px;" ></span> <font style="cursor:pointer; border:solid 1px #cecece; padding:3px; font-size:12px;">show</font> </label></h3>
											<div class="col-md-12" id="eBookguide" style="font-style:italic; font-size:12px;">click the <strong>arrow</strong> or <strong>show</strong> button to display the created pages</div>
									<hr>
									</div>
								</div>
								-->
									...
									<?php if(!empty($mainPage)):?>
										...
										<div class="form-group" style="text-align:right; margin-right:10px;">
											...
											<?php
												...
												$chapter_counter = 1;
				                                $chapter_title = "Chapter ".$chapter_counter.": ".$mainPage['Topic']['title'];
				                                if(!$isMobile)
				                                echo '<a href="'.$this->Html->url(array('controller' => 'teachers', 'action' => 'mainconedit', $this->Detergent->urlsafe_b64encode($mainPage['Topic']['id']."|".$chapter_title))).'"><button class="btn btn-default">Edit</button></a>';
...
var roxyFileman = '/<?php echo $folderName;?>/app/webroot/js/ckeditor/plugins/fileman/index.html';
$(function(){
   CKEDITOR.replace( 'WewContent',{filebrowserBrowseUrl:roxyFileman,
	                                 filebrowserUploadUrl:roxyFileman,
	                                 filebrowserImageBrowseUrl:roxyFileman+'?type=image',
	                                 filebrowserImageUploadUrl:null,
	                                 filebrowserImageBrowseLinkUrl: false,
	                                 toolbar: 'MyUploadToolBar'
	                                });
